...
const smart_format = (timestamp: number, pos_sign?: string, full_format = false): string => {
...
export const format_time = (timestamp: number, empty_label: string = "--:--", pos_sign = ""): string => {
	if (timestamp !== -1)
		return smart_format(timestamp, pos_sign);
	return empty_label;
}

export const format_time_full = (timestamp: number, empty_label: string = "--:--", pos_sign = "", ): string => {
	if (timestamp !== -1)
